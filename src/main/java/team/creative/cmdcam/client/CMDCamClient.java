...
public class CMDCamClient {
    ...
    public static void commands(RegisterClientCommandsEvent event) {
        event.getDispatcher().register(LiteralArgumentBuilder.<CommandSourceStack>literal("cam").executes((x) -> {
            ...
            x.getSource()
                    .sendSuccess(new TextComponent("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam reload" + ChatFormatting.RED + "tries to reload the saved paths from disk"), false);
            x.getSource()
                    .sendSuccess(new TextComponent("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam gui <true|false>" + ChatFormatting.RED + "Enables (true) or disables (false) the GUI while traveling"), false);
            x.getSource()
                    .sendSuccess(new TextComponent("" + ChatFormatting.BOLD + ChatFormatting.YELLOW + "/cam mouse <true|false>" + ChatFormatting.RED + "Locks the mouse (true) or not (false) while traveling"), false);
            return 0;
            ...
            return 0;
                ...
                .then(LiteralArgumentBuilder.<CommandSourceStack>literal("reload").executes((x) -> {
                    if (CMDCamClient.isInstalledOnSever) {
                        // Ignoring
                        System.out.println("Server mode - ignoring reload...");
                    } else {
                        CMDCamClient.storage.load();
                        x.getSource().sendSuccess(new TextComponent("Reloaded successfully!"), false);
                    }
                    return 0;
                }))
                .then(LiteralArgumentBuilder.<CommandSourceStack>literal("gui")
                        .then(RequiredArgumentBuilder.<CommandSourceStack, Boolean>argument("show", BoolArgumentType.bool()).executes((x) -> {
                            boolean enabled = BoolArgumentType.getBool(x, "show");
                            CMDCamClient.enableGuiOnTravel = enabled;
                            x.getSource().sendSuccess(new TextComponent("GUI shown on travel:  '" + enabled + "'"), false);
                            return 0;
                        })))
                .then(LiteralArgumentBuilder.<CommandSourceStack>literal("mouse")
                        .then(RequiredArgumentBuilder.<CommandSourceStack, Boolean>argument("lock", BoolArgumentType.bool()).executes((x) -> {
                            boolean enabled = BoolArgumentType.getBool(x, "lock");
                            CMDCamClient.grabMouseOnTravel = enabled;
                            x.getSource().sendSuccess(new TextComponent("Mouse locked on travel:  '" + enabled + "'"), false);
                            return 0;
                        })))
            );
    }
